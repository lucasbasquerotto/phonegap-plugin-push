...
public class PushPlugin extends CordovaPlugin implements PushConstants {
	...
	private static JSONObject cachedMainData = null;
	private static List<JSONObject> cachedDataList = Collections.synchronizedList(new ArrayList<JSONObject>());
		...
		if (INITIALIZE.equals(action)) {
			cordova.getThreadPool().execute(new Runnable() {
				public void run() {
					...
					if (cachedMainData != null) {
						Log.v(LOG_TAG, "sending cachedMainData");
						JSONObject json = cachedMainData;
						cachedMainData = null;
						sendEvent(json);
					}

					if (!cachedDataList.isEmpty()) {
						Log.v(LOG_TAG, "sending cachedDataList");

						synchronized(cachedDataList) {
							Iterator<JSONObject> iterator = cachedDataList.iterator();
	...
	*/
	public static void sendExtras(Bundle extras) {
		if (extras != null) {
			...
			if (gWebView != null) {
				Log.v(LOG_TAG, "sendExtras - extrasToJSON: " + json.toString());
				sendEvent(json);
			} else if (!"1".equals(noCache)) {
				Log.v(LOG_TAG, "sendExtras: caching extras to send at a later time.");

				synchronized(cachedDataList) {
					cachedDataList.add(json);
				}
			}
		}
	}

	public static void sendMainData(Bundle lastExtras, List<Bundle> listExtras) {
		if (lastExtras != null) {
			String noCache = lastExtras.getString(NO_CACHE);
			JSONObject last = convertBundleToJson(lastExtras);
			List<JSONObject> list = new ArrayList<JSONObject>();
			Integer notId = 0;

			try {
				JSONObject additionalData = last.getJSONObject(ADDITIONAL_DATA);
				notId = (additionalData != null) ? additionalData.getInt(NOT_ID) : 0;
			} catch (JSONException e) {
				Log.e(LOG_TAG, "sendMainData: JSON exception -> " + e.getMessage());
			}

			notId = (notId != null) ? notId : 0;

			if (listExtras != null) {
				for (Bundle extras : listExtras) {
					JSONObject item = convertBundleToJson(extras);
					list.add(item);
				}
			}
			...
			try {
				...
				json.put(NOT_ID, notId);
				json.put(PUSH_MAIN_DATA_LAST, last);
				json.put(PUSH_MAIN_DATA_LIST, new JSONArray(list));

				Log.d(LOG_TAG, "sendMainData: " + json + " - " + gWebView);
